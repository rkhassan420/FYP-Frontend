...
import {
  ...
  AlertCircle, X, FileCheck, ArrowLeft, ClipboardList, Ban, Loader2,
} from "lucide-react";
...
function clearProcStart(submissionId) {
  try { localStorage.removeItem(PROC_START_KEY + submissionId); }
  catch { /* ignore */ }
}
...
function ProcessingLoader({ isComplete, percentage, submissionId }) {
  // Stable start time — read from localStorage so it survives reloads/navigation
  const [startTime] = useState(() => getOrCreateProcStart(submissionId));
  const [elapsed, setElapsed] = useState(() => Date.now() - startTime);
  const [msgIdx, setMsgIdx] = useState(0);

  // Deterministic fake % derived from elapsed time (not random state).
  // Approaches 90% asymptotically — same value on every reload for same elapsed.
  const elapsedSec = elapsed / 1000;
  const fakePct = Math.min(90, Math.round(90 * (1 - Math.exp(-elapsedSec / 60))));

  useEffect(() => {
    if (isComplete) {
      // Clean up stored start time once analysis is done
  ...
  }, [isComplete, startTime, submissionId]);
...
function AssignmentDetail({ assignment, onBack, onSubmitSuccess }) {
  ...
  // Stable submission ID for this assignment (from localStorage)
  ...
  const handleFile = (file) => {
    ...
    if (!["application/pdf"].includes(file.type) && fileExtension !== ".pdf") {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Initialise the persistent timer immediately on successful submission
              ...
              <div className="meta-chips">
                {assignment.max_score && (
                  <div className="meta-chip">
                    <span className="meta-chip-label">Max Score</span>
                    <span className="meta-chip-value">{assignment.max_score}</span>
                  </div>
                )}
          ...
          <div className="submission-panel">
            ...
            {isBlocked && assignment.status !== "submitted" ? (
              ...
              <div className="submitted-state">
                ...
                {/* ── Animated processing loader (persistent across reloads) ── */}
...
// ─── Root Export ──────────────────────────────────────────────────────────────
export default function StudentAssignments({ selectedClass, setActivePage }) {
    ...
    <div className="page-wrapper">
      <div className="page-inner" style={{ maxWidth: "860px" }}>
        <header className="page-header fade-in">
          {selectedClass && (
            <button className="back-btn" onClick={() => setActivePage?.("my-classes")}>
