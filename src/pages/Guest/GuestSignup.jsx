...
const getFirstError = (errorValue) => {
  if (Array.isArray(errorValue) && errorValue.length > 0) return errorValue[0];
...
const GuestSignup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/guest/verify-otp", { state: { identifier: email, flow: "signup" } });
      }, 1200);
    } catch (err) {
          ...
          {!success ? (
            <>
              ...
              <p className="signup-link">
                ...
                <span onClick={() => navigate("/guest/login")}>Login</span>
              </p>
            </>
          ) : (
